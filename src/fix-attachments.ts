...
async function fixAttachments() {
  try {
    console.log("Fixing attachment foreign key issues...");
    ...
    // Drop and recreate the attachments table
    console.log("Dropping attachments table...");
    await sequelize.getQueryInterface().dropTable('attachments');
